...
const headCells = [
  ...
  { id: "title", numeric: false, disablePadding: false, label: "Cím" },
        ...
        for (let i = 0; i < response.data.rows.length; i++) {
          ...
          for (let j = 0; j < props.numSelected.length; j++) {
            ...
            if (element2 === element.id) {
              selectedJobs.push(element);
            }
          }
        }
        const csvExporter = new ExportToCsv(options);
        csvExporter.generateCsv(selectedJobs);
      })
...
function EnhancedTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoaded(false);
      try {
        const [jobs] = await axios.all([axios.get(API_URL + "/jobs")]);
        setData(jobs.data);
      } catch (error) {
        setError(error);
      }
